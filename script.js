window.addEventListener('load', ()=> {
    ...
    nameInput.addEventListener('change', e => {             //listenes to any changes that is made
        localStorage.setItem('username', e.target.value);   //the value changes(=the inputted name in this case)
    })
...
function DisplayTodos () {      //this is where we will "push" our new todos
    ...
    todos.forEach(todo => {
        ...
        if(todo.category == 'personal') {       //if the category is personal, it adds to the 'personal' class to it
        ...
        content.innerHTML = `<input type="text" value="${todo.content}"     //here we change the content of the content input
        readonly>`;     
        edit.innerHTML = 'edit';
        ...
        input.addEventListener('click', e => {      //creating an eventListener on click
        ...
        edit.addEventListener('click', e => {               //if we click on this item...
            ...
            input.addEventListener('blur', e => {     //here it is setted, that if you click outside the input field, it will stop editing
